 ...
 */
final class HummingbirdStateImpl implements HummingbirdState
   {
   ...
   private final short[] analogInputs = new short[HummingbirdConstants.ANALOG_INPUT_DEVICE_COUNT];
    ...
    */
   static HummingbirdStateImpl create(final byte[] state)
      {
      if (state == null)
         {
   ...
   private HummingbirdStateImpl(final byte[] state)
      ...
      for (int i = 0; i < analogInputs.length; i++)
         {
         this.analogInputs[i] = (short)ByteUtils.unsignedByteToInt(state[20 + i]);
         }
      }
   ...
   public int hashCode()
      {
      int result;
      result = (orbs != null ? Arrays.hashCode(orbs) : 0);
