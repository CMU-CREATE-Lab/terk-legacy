...
import edu.cmu.ri.createlab.TeRK.client.robotdiaries.conditions.ConditionEvaluator;
import edu.cmu.ri.createlab.TeRK.client.robotdiaries.expressions.ExpressionLoader;
...
public class SequencePlayer
   ...
   private SequencePlayer embeddedPlayer;
   ...
   public void playSequence(ServiceManager serviceManager, Sequence sequence)
      {
      embeddedPlayer = new SequencePlayer();
      ...
      try
         ...
         while (!stopPlaying)
            ...
            if (executeNextStep)
               ...
               Object step = currentStep.getStep();
               if (step instanceof XmlExpression)
                  {
                  expressionLoader.executeToServices(serviceManager, (XmlExpression)step);
                  stepTimeStart = Calendar.getInstance().getTimeInMillis();
                  }
               else if (step instanceof Sequence)
                  {
                  Sequence seq = (Sequence)step;
                  seq.setLoopBackToStart(false);
                  SequenceTransition zeroTime = new SequenceTransition();
                  zeroTime.setCondition(null);
                  zeroTime.setMillisToNextStep(0);

                  if (seq.getSteps().size() > 0)
                     {
                     ((SequenceStep)seq.lastElement()).setTransition(zeroTime);
                     }

                  this.embeddedPlayer.playSequence(serviceManager, (Sequence)step);
                  stepTimeStart = Calendar.getInstance().getTimeInMillis();
                  }
               index++;
