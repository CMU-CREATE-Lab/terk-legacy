...
public class SequenceFileHandler extends AbstractFileHandler<Sequence>
   ...
   public Hashtable getRepresentation(Sequence sequence)
      ...
      for (SequenceStep step : sequence.getSteps())
         ...
         // Store XmlExpression or Sequence
         if (step.getStep() instanceof XmlExpression)
            {
            stepHash.put(EXPRESSION_KEY, expressionFileHandler.getRepresentation((XmlExpression)step.getStep()));
            }
         else if (step.getStep() instanceof Sequence)
            {
            stepHash.put(SEQUENCE_KEY, this.getRepresentation((Sequence)step.getStep()));
            }
         // Store Transition
   ...
   public Sequence setRepresentation(final Hashtable hash)
      ...
      for (Object o : stepList)
         ...
         if (tempObject != null && tempObject instanceof Hashtable)
            ...
            newStep = new SequenceStep<XmlExpression>(e);
            }
         else
            {
            tempObject = stepHash.get(SEQUENCE_KEY);
            if (tempObject != null && tempObject instanceof Hashtable)
               {
               Sequence s = this.setRepresentation((Hashtable)tempObject);
               if (s == null)
                  {
                  continue;
                  }

               newStep = new SequenceStep<Sequence>(s);
               }
            else
               {
               continue;
